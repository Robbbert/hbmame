...
public:
  ...
  explicit CodeHolder(Span<uint8_t> static_arena_memory = Span<uint8_t>{}) noexcept;
  ...
  Error init(const Environment& environment, uint64_t base_address = Globals::kNoBaseAddress) noexcept;
  //! Initializes CodeHolder to hold code described by the given `environment`, `cpu_features`, and `base_address`.
  Error init(const Environment& environment, const CpuFeatures& cpu_features, uint64_t base_address = Globals::kNoBaseAddress) noexcept;
  ...
  Error reinit() noexcept;

  //! Detaches all code-generators attached and resets the `CodeHolder`.
  void reset(ResetPolicy reset_policy = ResetPolicy::kSoft) noexcept;
  ...
  Error grow_buffer(CodeBuffer* cb, size_t n) noexcept;

  //! Reserves the size of `cb` to at least `n` bytes.
  //!
  //! \note The buffer `cb` must be managed by `CodeHolder` - otherwise the behavior of the function is undefined.
  Error reserve_buffer(CodeBuffer* cb, size_t n) noexcept;
  ...
  Error new_section(Out<Section*> section_out, const char* name, size_t name_size = SIZE_MAX, SectionFlags flags = SectionFlags::kNone, uint32_t alignment = 1, int32_t order = 0) noexcept;
  ...
  [[nodiscard]]
  Section* section_by_name(const char* name, size_t name_size = SIZE_MAX) const noexcept;
  ...
  [[nodiscard]]
  Section* ensure_address_table_section() noexcept;
  ...
  Error add_address_to_address_table(uint64_t address) noexcept;
  ...
  [[nodiscard]]
  Error new_label_id(Out<uint32_t> label_id_out) noexcept;
  ...
  //! name.
  [[nodiscard]]
  Error new_named_label_id(Out<uint32_t> label_id_out, const char* name, size_t name_size, LabelType type, uint32_t parent_id = Globals::kInvalidId) noexcept;
  ...
  [[nodiscard]]
  uint32_t label_id_by_name(const char* name, size_t name_size = SIZE_MAX, uint32_t parent_id = Globals::kInvalidId) noexcept;

  //! \overload
  [[nodiscard]]
  uint32_t label_id_by_name(Span<const char> name, uint32_t parent_id = Globals::kInvalidId) noexcept {
  ...
  [[nodiscard]]
  Fixup* new_fixup(LabelEntry& le, uint32_t section_id, size_t offset, intptr_t rel, const OffsetFormat& format) noexcept;
  ...
  Error resolve_cross_section_fixups() noexcept;
  ...
  Error bind_label(const Label& label, uint32_t section_id, uint64_t offset) noexcept;
  ...
  [[nodiscard]]
  Error new_reloc_entry(Out<RelocEntry*> dst, RelocType reloc_type) noexcept;
  ...
  //!
  //! \note This should never be called more than once.
  Error relocate_to_base(uint64_t base_address, RelocationSummary* summary_out = nullptr) noexcept;

  //! Copies a single section into `dst`.
  Error copy_section_data(void* dst, size_t dst_size, uint32_t section_id, CopySectionFlags copy_flags = CopySectionFlags::kNone) noexcept;
  ...
  Error copy_flattened_data(void* dst, size_t dst_size, CopySectionFlags copy_flags = CopySectionFlags::kNone) noexcept;
