...
public:
  ...
  explicit BaseEmitter(EmitterType emitter_type) noexcept;
  virtual ~BaseEmitter() noexcept;
  ...
  void set_error_handler(ErrorHandler* error_handler) noexcept;

  //! Resets the error handler.
  ASMJIT_INLINE_NODEBUG void reset_error_handler() noexcept { set_error_handler(nullptr); }

  //! \cond INTERNAL
  Error _report_error(Error err, const char* message = nullptr);
  //! \endcond
  ...
  void add_diagnostic_options(DiagnosticOptions options) noexcept;

  //! Deactivates the given validation `options`.
  //!
  //! See \ref add_diagnostic_options() and \ref DiagnosticOptions for more details.
  void clear_diagnostic_options(DiagnosticOptions options) noexcept;
  ...
  virtual Error section(Section* section);
  ...
  [[nodiscard]]
  virtual Label new_label();

  //! Creates a new named label.
  [[nodiscard]]
  virtual Label new_named_label(const char* name, size_t name_size = SIZE_MAX, LabelType type = LabelType::kGlobal, uint32_t parent_id = Globals::kInvalidId);
  ...
  [[nodiscard]]
  Label label_by_name(const char* name, size_t name_size = SIZE_MAX, uint32_t parent_id = Globals::kInvalidId) noexcept;

  //! \overload
  [[nodiscard]]
  Label label_by_name(Span<const char> name, uint32_t parent_id = Globals::kInvalidId) noexcept {
  ...
  virtual Error bind(const Label& label);

  //! Tests whether the label `id` is valid (i.e. registered).
  [[nodiscard]]
  bool is_label_valid(uint32_t label_id) const noexcept;
  ...
  Error _emitI(InstId inst_id);
  //! \overload
  Error _emitI(InstId inst_id, const Operand_& o0);
  //! \overload
  Error _emitI(InstId inst_id, const Operand_& o0, const Operand_& o1);
  //! \overload
  Error _emitI(InstId inst_id, const Operand_& o0, const Operand_& o1, const Operand_& o2);
  //! \overload
  Error _emitI(InstId inst_id, const Operand_& o0, const Operand_& o1, const Operand_& o2, const Operand_& o3);
  //! \overload
  Error _emitI(InstId inst_id, const Operand_& o0, const Operand_& o1, const Operand_& o2, const Operand_& o3, const Operand_& o4);
  //! \overload
  Error _emitI(InstId inst_id, const Operand_& o0, const Operand_& o1, const Operand_& o2, const Operand_& o3, const Operand_& o4, const Operand_& o5);
  ...
  virtual Error _emit(InstId inst_id, const Operand_& o0, const Operand_& o1, const Operand_& o2, const Operand_* op_ext);
  //! Emits instruction having operands stored in array.
  virtual Error _emit_op_array(InstId inst_id, const Operand_* operands, size_t op_count);
  ...
  virtual Error align(AlignMode align_mode, uint32_t alignment);
  ...
  virtual Error embed(const void* data, size_t data_size);
  ...
  virtual Error embed_data_array(TypeId type_id, const void* data, size_t item_count, size_t repeat_count = 1);
  ...
  virtual Error embed_const_pool(const Label& label, const ConstPool& pool);
  ...
  virtual Error embed_label(const Label& label, size_t data_size = 0);
  ...
  virtual Error embed_label_delta(const Label& label, const Label& base, size_t data_size = 0);
  ...
  virtual Error comment(const char* data, size_t size = SIZE_MAX);
  ...
  virtual Error on_attach(CodeHolder& code) noexcept;

  //! Called after the emitter was detached from `CodeHolder`.
  virtual Error on_detach(CodeHolder& code) noexcept;

  //! Called when CodeHolder is reinitialized when the emitter is attached.
  virtual Error on_reinit(CodeHolder& code) noexcept;
  ...
  virtual void on_settings_updated() noexcept;
