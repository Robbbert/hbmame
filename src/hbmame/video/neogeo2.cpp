...
void neogeo2_state::create_rgb_lookups()
{
...
void neogeo2_state::neogeo_set_screen_shadow( int data )
{
...
void neogeo2_state::neogeo_set_palette_bank( int data )
{
...
READ16_MEMBER(neogeo2_state::neogeo_paletteram_r)
{
	return m_paletteram[m_palette_bank + offset];
}


WRITE16_MEMBER(neogeo2_state::neogeo_paletteram_w)
{
...
void neogeo2_state::video_start()
{
	create_rgb_lookups();
	...
	machine().save().register_postload(save_prepost_delegate(FUNC(neogeo2_state::set_pens), this));
...
void neogeo2_state::video_reset()
{
}
...
uint32_t neogeo2_state::screen_update_neogeo(screen_device &screen, bitmap_rgb32 &bitmap, const rectangle &cliprect)
{
...
uint16_t neogeo2_state::get_video_control(  )
{
	uint16_t ret;
	uint16_t v_counter;
...
void neogeo2_state::set_video_control( uint16_t data )
{
...
READ16_MEMBER(neogeo2_state::neogeo_video_register_r)
{
	uint16_t ret;
...
WRITE16_MEMBER(neogeo2_state::neogeo_video_register_w)
{
	/* accessing the LSB only is not mapped */
	if (mem_mask != 0x00ff)
	{
